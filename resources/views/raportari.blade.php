...
<div class="firstScreen parallax header-mic" style = "background-attachment: inherit;background-image:url({{ route('thumb', ['width:1920', setting('detaliu.adunari-banner')]) }})">
  ...
  <div class="overFirst">
    <div class="title">
    <div class="titleInside">{{ __('site.raportari') }}</div>
    </div>
...
<div class = "services">
  ...
  <div class = "categorii">
    <div class="swiper-container">
    <div class="swiper-wrapper">
      @foreach($agauri as $get)
      <div class="swiper-slide"><div class="categorie-element uppercase-text" id_aga="{{$get['id']}}">{{$get['name']}}</div></div>
      @endforeach
    </div>
  </div>
  </div>
  ...
  <div class = "lista-fisiere">
    @if($gatherings->count() >0)
       @foreach($gatherings as $gathering)
          <div class = "fisier fisier-an-{{$gathering['ani']}} fisier-aga-{{$gathering['id_aga']}}" an_curent_fisier="{{$gathering['ani']}}" aga_curent_fisier="{{$gathering['id_aga']}}">
